...
// 	    }
	    PHINode* exit_phi = dyn_cast<PHINode>(exit_bb->begin());
	    if(!exit_phi)
	    {
		outs()<<"the exit BB don't has PHINode..\n";
	    }
	    IRBuilder<> builder(context);
	    
// 	    Value* phi_value = exit_phi->getIncomingValueForBlock(exit_bb);
	    string s;
	    for(unsigned int n=0;n<exit_phi->getNumOperands();n++)
	    {
// 		outs()<<"n = "<<n<<"\n";
		string str;
		BasicBlock* incomingBB = exit_phi->getIncomingBlock(n);
		BasicBlock* incomingsuccBB;
		if(incomingBB->getTerminator()->getSuccessor(0)!=exit_bb)
		{
		    incomingsuccBB = incomingBB->getTerminator()->getSuccessor(0);
		}
		else
		{
		    incomingsuccBB = incomingBB->getTerminator()->getSuccessor(1);
		}
		BranchInst* brinst = dyn_cast<BranchInst>(incomingBB->getTerminator());
		str = "$"+incomingBB->getName().str()+" "+calculateVarible(brinst->getCondition());
		incomingBB->getTerminator()->dropAllReferences();
		incomingBB->getTerminator()->removeFromParent();
		builder.SetInsertPoint(incomingBB);
		if(incomingBB!=latch)
		{
		    s = s + str;
		    builder.CreateBr(incomingsuccBB);
		    exit_phi->removeIncomingValue(incomingBB);
		    n--;
		}
		else
		{
	    ...
	    if(!s.empty())
		s = "(assert (=> $"+header->getName().str()+" (and "+s+")))";
	    
	    end_formula = s;
// 	    Value* phi_value = exit_phi->getIncomingValueForBlock(exit_bb);
// 	    for(unsigned int n=exit_phi->getNumOperands();n!=0;n--)
// 	    {
// 		exit_phi->removeIncomingValue(0);
// 	    }
// 	    exit_phi->addIncoming(phi_value,latch);
...
// 		full_formula = full_formula + s ;
// 	    }else 
	    if(basicblock->getName().str()=="entry")
	    {
		...
		string s;
		int n=0;
		for(pred_iterator itr=pred_begin(basicblock);itr!=pred_end(basicblock);itr++)
		{
		    BasicBlock *pred=*itr;
			...
			{
			    s = s + "$"+pred->getName().str()+" ";
			}
			else
			{
			    if(pred->getTerminator()->getSuccessor(0)==basicblock)
			    {
				s = s + "(and $"+pred->getName().str()+" "+calculateVarible(br->getCondition())+")";
			    }
			    else
			    {
				s = s + "(and $"+pred->getName().str()+" (not "+calculateVarible(br->getCondition())+"))";
			    }
			}
		    }
		    else
		    {
			SwitchInst *sw = dyn_cast<SwitchInst>(pred->getTerminator());
			//need to deal it
		    }
		    n++;
		}
		...
		if(n>1)
		{
		    s = "(assert (= (or "+s+") $"+basicblock->getName().str()+"))";
		}
		else
		{
		    s = "(assert (= "+s+" $"+basicblock->getName().str()+"))";
		}
		full_formula = full_formula + s;
		outs()<<s<<'\n';
	    }
		...
		switch(inst_i->getOpcode())
		{
			...
			string s = "(assert (= $"+phi->getName().str()+" ";
			string tail = " ) )";
			
			for(unsigned int num =0;num < NumForLoop ;num++)
			{
			    s = s+"(ite $"+phi->getIncomingBlock(num)->getName().str()+" "+calculateVarible(phi->getIncomingValue(num))+" ";
			    tail = tail+" )";
			}
			...
			vector<string> strs;
			string str;
			for(unsigned int n=0;n<NumOperand;n++)
			{
			    BranchInst* br = dyn_cast<BranchInst>(phi->getIncomingBlock(n)->getTerminator());
			    if(br)
			    {
				if(br->getNumSuccessors()==1)
				{
				    str ="$" + br->getParent()->getName().str();
				    strs.push_back(str);
				}
				else
				{
				    if(br->getSuccessor(0)==basicblock)
				    {
					str ="(and "+calculateVarible(br->getCondition())+"$"+br->getParent()->getName().str()+")";
					strs.push_back(str);
				    }
				    else if(br->getSuccessor(1)==basicblock)
				    {
					str ="(and (not "+calculateVarible(br->getCondition())+") $"+br->getParent()->getName().str()+")";
					strs.push_back(str);
				    }
				}
			    }
			    
			    SwitchInst* sw = dyn_cast<SwitchInst>(phi->getIncomingBlock(n)->getTerminator());
			    if(sw)
			    {
				Value* Cond = sw->getCondition();
				string Cond_str = calculateVarible(Cond);
				string default_subStr = "(and $"+basicblock->getName().str();
				...
				strs.push_back(default_subStr);
			    }
			}
			
			s = "(assert (=> $"+basicblock->getName().str()+" (or ";
// 			outs()<<NumOperand<<"\n";
			for(unsigned int i=0;i<NumOperand;i++)
			{
			    string part="(and ";
			    vector<string>::iterator RI = strs.begin();
			    for(unsigned int j=0;j<NumOperand;j++)
			    {
				if(i==j)
				{
				    part = part + (*RI)+" ";
...
// 			    outs()<<part<<"\n";
			...
			*/
			...
			if(rbitwidth > 1)
			{
			    string s = "(assert (= $"+inst_i->getName().str()+"((_ sign_extend "+InttoString(lbitwidth-rbitwidth)+") "+calculateVarible(inst_i->getOperand(0))+")))";
			    full_formula = full_formula + s;
			    outs()<<s<<'\n';
			    break;
			}else
...
/*	    unsigned int numSucc = basicblock->getTerminator()->getNumSuccessors();
		...
		case Instruction::Br:
		{
			...
			s =s+ "(assert (=> $"+bbi_name+" $"+bbsucc_name+") )";
			full_formula = full_formula +s;
			outs()<<s+'\n';
		    }else
		    {
			...
			s = "(assert (=> $"+bbi_name+" (and (=> "+calculateVarible(brInst->getCondition())+" $"+bbsucc1_name+") "
			    +"(=> (not "+calculateVarible(brInst->getCondition())+") $"+bbsucc2_name+"))))";
// 			s =s+ "(assert (and "+
// 				"(=> (and $"+bbi_name+" "+calculateVarible(brInst->getCondition())+") $"+bbsucc1_name+") "+
// 				"(=> (and $"+bbi_name+" (not "+calculateVarible(brInst->getCondition())+") ) $"+bbsucc2_name+") ) )";
			full_formula = full_formula +s;
			outs()<<s+'\n';
		    }
		    break;
